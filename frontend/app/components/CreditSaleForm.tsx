...
export default function CreditSaleForm({
  ...
  const [saleType, setSaleType] = useState<"PARTIALLY_PAID" | "CREDITED">(
    "CREDITED",
  );
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setErrorMsg("Error processing sale.");
    } finally {
      setLoading(false);
    }
  }
  ...
  const handleScanAt = (index: number, sku: string) => {
    ...
    if (cart.some((c) => c.productId === String(product.id))) {
      setErrorMsg(
        `${product.brand} ${product.baseName} is already in the sale`,
      );
      return;
    }
    setCart((prev) =>
    ...
    <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-4">
      ...
      <div>
        ...
        <div className="flex gap-2">
          <button
            type="button"
            onClick={() => setSaleType("CREDITED")}
            className={
              saleType === "CREDITED"
                ? "flex-1 py-2 rounded-lg text-sm font-medium border bg-red-500 text-white border-red-500"
                : "flex-1 py-2 rounded-lg text-sm font-medium border bg-white text-gray-600 border-gray-300"
            }
          >
            Full Credit
          </button>
          <button
            type="button"
            onClick={() => setSaleType("PARTIALLY_PAID")}
            className={
              saleType === "PARTIALLY_PAID"
                ? "flex-1 py-2 rounded-lg text-sm font-medium border bg-amber-500 text-white border-amber-500"
                : "flex-1 py-2 rounded-lg text-sm font-medium border bg-white text-gray-600 border-gray-300"
            }
          >
            Partial
        ...
        <div key={idx} className="border p-3 rounded-lg flex gap-2">
          <div className="flex-1">
            ...
            <div className="flex gap-2">
              <select
                ...
                onChange={(e) => {
                  const nc = [...cart];
                  nc[idx].productId = e.target.value;
                  setCart(nc);
                }}
          ...
          <div className="w-20">
            ...
            <input
              ...
              onChange={(e) => {
                const nc = [...cart];
                nc[idx].quantity = Number(e.target.value);
                setCart(nc);
              }}
          ...
          <div className="w-24">
            ...
            <input
              ...
              onChange={(e) => {
                const nc = [...cart];
                nc[idx].customPrice = e.target.value;
                setCart(nc);
              }}
              className="border p-2 rounded-lg w-full text-sm"
            />
      ...
      <div className="flex gap-2">
        <button
          type="button"
          onClick={() =>
            setCart([...cart, { productId: "", quantity: 1, customPrice: "" }])
          }
